...
If ($ENV:PROCESSOR_ARCHITEW6432 -eq "AMD64") {
    Try {
        &"$ENV:WINDIR\SysNative\WindowsPowershell\v1.0\PowerShell.exe" -File $PSCOMMANDPATH -DeviceUUID $DeviceUUID -DriverName $DriverName -PrinterName $PrinterName -INFFile $INFFile
    }
    Catch {
...
Write-LogEntry -Value "DeviceUUID: $DeviceUUID"
...
If (-not $ThrowBad) {
    ...
    Try {
        ...
        Write-LogEntry -Stamp -Value "Running command: Start-Process pnputil.exe $($INFARGS) -wait -passthru"
...
If (-not $ThrowBad) {
    Try {
        ...
        if (-not $PrinterExist) {
            ...
            Add-Printer -Name $PrinterName -DriverName $DriverName -DeviceUUID $DeviceUUID -Confirm:$false
        }
        else {
